...
function shouldAutoUpdate({ flags, cliConfig }) {
...
async function buildSanityStudio(args, context, overrides) {
  ...
  if (didInstall)
    return { didCompile: !1 };
  const autoUpdatesEnabled = shouldAutoUpdate({ flags, cliConfig }), coercedSanityVersion = semver__default.default.coerce(installedSanityVersion)?.version;
