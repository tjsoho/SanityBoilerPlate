...
async function deployStudioAction(args, context) {
  const { apiClient, workDir, chalk, output, prompt, cliConfig } = context, flags = { build: !0, ...args.extOptions }, customSourceDir = args.argsWithoutOptions[0], sourceDir = path__default.default.resolve(process.cwd(), customSourceDir || path__default.default.join(workDir, "dist")), isAutoUpdating = buildAction.shouldAutoUpdate({ flags, cliConfig }), installedSanityVersion = await helpers.getInstalledSanityVersion(), configStudioHost = cliConfig && "studioHost" in cliConfig && cliConfig.studioHost, client = apiClient({
    requireUser: !0,
    requireProject: !0
...
`);
  ...
  if (flags.build) {
    const buildArgs = [customSourceDir].filter(Boolean), { didCompile } = await buildAction.buildSanityStudio(
