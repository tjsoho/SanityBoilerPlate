...
import { fromEvent, timer, EMPTY, of, merge, map as map$1, startWith as startWith$1, catchError as catchError$1, from, concat, firstValueFrom, filter, BehaviorSubject, defer, mergeMap, throwError, share, partition, take, throttleTime, asyncScheduler, Subject, tap as tap$1, debounce } from "rxjs";
...
function RequestPermissionDialog({
  ...
  const { t } = useTranslation(structureLocaleNamespace), telemtry = useTelemetry(), dialogId = `request-permissions-${useId()}`, projectId = useProjectId(), client = useClient({ apiVersion: "2024-09-26" }), toast = useToast(), zOffset = useZIndex(), [isSubmitting, setIsSubmitting] = useState(!1), [note, setNote] = useState(""), [noteLength, setNoteLength] = useState(0), [msgError, setMsgError] = useState(), [hasTooManyRequests, setHasTooManyRequests] = useState(!1), [hasBeenDenied, setHasBeenDenied] = useState(!1), requestedRole$ = useMemo(() => {
        ...
        hasTooManyRequests || hasBeenDenied ? /* @__PURE__ */ jsx(Card, { tone: "caution", padding: 3, radius: 2, shadow: 1, children: /* @__PURE__ */ jsxs(Text, { size: 1, children: [
          ...
          /* @__PURE__ */ jsx(
            TextInput,
            {
              ...
              value: note,
              onChange: (e) => {
                setNote(e.currentTarget.value), setNoteLength(e.currentTarget.value.length);
              }
            }
          ),
          /* @__PURE__ */ jsx(Text, { align: "right", muted: !0, size: 1, children: `${noteLength}/${MAX_NOTE_LENGTH}` })
        ] })
      ] }) })
    }
  ) });
